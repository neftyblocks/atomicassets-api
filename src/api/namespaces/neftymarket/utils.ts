...
export function buildAuctionFilter(values: FilterValues, query: QueryBuilder): void {
    ...
    if (args.symbol) {
        ...
        if (args.min_buy_now_price) {
            query.addCondition('price.buy_now_price_dynamic >= 1.0 * ' + query.addVariable(args.min_buy_now_price) + ' * POWER(10, "token".token_precision)');
        }

        if (args.max_buy_now_price) {
            query.addCondition('price.buy_now_price_dynamic <= 1.0 * ' + query.addVariable(args.max_buy_now_price) + ' * POWER(10, "token".token_precision)');
        }
